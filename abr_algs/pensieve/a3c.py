...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.entropy_weight = 5.0

        self.tf_entropy_weight = tf.placeholder(tf.float32, shape = ())
        ...
        self.obj = tf.reduce_sum(tf.multiply(
                   ...
                   + self.tf_entropy_weight * tf.reduce_sum(tf.multiply(self.out,
    ...
    def update_entropy(self, val):
        self.entropy_weight = val
        # # print 'UPDATING WITH ENTROPY '+str(ENTROPY_WEIGHT)
        # self.obj = tf.reduce_sum(tf.multiply(
        #     tf.log(tf.reduce_sum(tf.multiply(self.out, self.acts),
        #                          reduction_indices=1, keep_dims=True)),
        #     -self.act_grad_weights)) \
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            #merge_net = tflearn.merge([split_0, split_1, split_2_flat, split_3_flat, split_4_flat], 'concat')
    ...
    def train(self, inputs, acts, act_grad_weights):
        ...
        self.sess.run(self.optimize, feed_dict={
            ...
            self.tf_entropy_weight: self.entropy_weight
        })
